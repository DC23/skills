...
```
Confirm: it fails (not errors), for the right reason — feature missing, not a typo or import problem. Do not proceed until the human confirms the failure.

**GREEN** — write the minimal code to pass that test. Nothing more. Resist adding anything the test doesn't require — no extra methods, no anticipated cases. Human runs the full suite:
```
pytest -x
```
